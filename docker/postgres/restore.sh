...
if dropdb -h mcod-db -U $POSTGRES_USER $POSTGRES_DB
...
createdb -h mcod-db -U $POSTGRES_USER $POSTGRES_DB -O $POSTGRES_USER

# restore the database
echo "restoring database $POSTGRES_DB"
gunzip -c $BACKUPFILE | psql -h mcod-db -U $POSTGRES_USER
