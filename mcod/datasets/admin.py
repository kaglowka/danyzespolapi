...
class DatasetAdmin(HistoryMixin, admin.ModelAdmin):
    actions_on_top = True
    autocomplete_fields = ['tags', 'organization']
    # prepopulated_fields = {"slug": ("title",)}
    list_display = ['title', 'organization', 'created_by', 'last_modified_resource_date', 'status', 'obj_history']
    ...
    def get_queryset(self, request):
        ...
        if request.method == 'GET':
            qs = qs.annotate(
                _last_modified_resource_date=Coalesce(Max('resources__modified'),
                                                      Value(datetime.datetime(1, 1, 1, 0, 0, 0)))
            )
            qs = qs.order_by('-_last_modified_resource_date')
        if request.user.is_superuser:
            return qs
        return qs.filter(organization_id__in=request.user.organizations.all())

    def last_modified_resource_date(self, obj):
        return obj._last_modified_resource_date

    last_modified_resource_date.admin_order_field = '_last_modified_resource_date'
    last_modified_resource_date.short_description = _('modified')
