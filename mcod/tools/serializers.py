...
class StatsBucketItem(BucketItem):
    key = ma.fields.Raw(dump_to='id')
    doc_count = ma.fields.Integer(dump_to='count')


class ByMonthItem(StatsBucketItem):
    title = ma.fields.String(dump_to='date', attribute='key_as_string')


class DocumentsByTypeBucketItem(StatsBucketItem):
    title = ma.fields.String(dump_to='type')
