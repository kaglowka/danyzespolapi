...
def test_email_unique():
    with pytest.raises(django.core.exceptions.ValidationError) as e:
        User.objects.create_user('aaa@example.com', 'P4n.Samochodzik:)')
        User.objects.create_user('aaa@example.com', 'P4n.Samochodzik:)')
...
def test_is_active(active_user):
    assert active_user.state is 'active'
...
def test_check_session_valid(mocker):
    usr = User.objects.create_user('aaa@example.com', 'P4n.Samochodzik:)')
...
class TestLogin(object):
    ...
    def test_active_user_cant_login_to_admin_panel(self, active_user):
        ...
        assert response.status_code == 302
        assert response.url == '/login/?next=/'
        client.login(email=active_user.email, password="P4n.Samochodzik:)")
        response = client.get("/")
        assert response.status_code == 302
