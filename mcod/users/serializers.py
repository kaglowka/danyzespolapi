...
class UserSchema(ja.Schema):
    ...
    state = ma.fields.Str(required=True, error_messages=str_error_messages,
                          faker_type='userstate', example='active',
                          description="Allowed values: 'active', 'inactive' or 'blocked'")
    email = ma.fields.Email(required=True, error_messages=email_error_messages,
                            faker_type='email', example='user@example.com')
    fullname = ma.fields.Str(missing=None, error_messages=str_error_messages,
                             faker_type='name', example='Jan Kowalski')
    about = ma.fields.Str(required=False, missing=None, faker_type='sentence',
...
class LoginSchema(UserSchema):
    token = ma.fields.Str(required=True, error_messages=str_error_messages, faker_type='uuid4',
