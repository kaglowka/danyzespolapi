...
from mcod.lib.search.helpers import extract_script_field
...
class FollowedListHandler(SearchHandler):
    ...
    def _data(self, request, cleaned, *args, **kwargs):
        queryset = self._queryset(cleaned, user_id=request.user.id, *args, **kwargs)
...
class FollowingSearchHandler(SearchHandler):
    ...
    def _queryset(self, cleaned, user_id, *args, **kwargs):
        ...
        return queryset.extra(stored_fields=['_source']).script_fields(followed=script)
