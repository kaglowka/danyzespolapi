...
from mcod.lib.parsers import Parser


class BaseHandler:
    def run(self, request, response, *args, **kwargs):
    ...
    def _clean(self, request, *args, locations=None, **kwargs):
        ...
        return Parser(locations=locations).parse(self.request_schema, req=request)
    ...
    def _serialize(self, data, meta, links=None, *args, **kwargs):
        res, errors = self.response_serializer.dump(data, meta, links)
        return res

    @classmethod
    def _docs_context(cls):
        return {
            'xxxx': 'Jakaś wartość z handlera'
        }
...
class SearchHandler(ListHandler):
    def _data(self, request, cleaned, *args, **kwargs):
        queryset = self._queryset(cleaned, *args, **kwargs)
    ...
    def _queryset(self, cleaned, *args, **kwargs):
        ...
        for name, field in self.request_schema.fields.items():
