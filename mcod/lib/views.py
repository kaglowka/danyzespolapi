...
class BaseApiView(metaclass=ApiViewMeta):
    ...
    def handle(self, req, resp, handler, *args, **kwargs):
        ...
        body = handler().run(req, resp, *args, **kwargs)
        resp.body = self.prepare_body(resp, body)
        resp.status = falcon.HTTP_200

    def prepare_body(self, resp, body):
        return json.dumps(body, cls=DateTimeToISOEncoder)

    @classmethod
    def _docs_context(cls):
        return {
            'aaaa': 'Jakaś wartość z widoku'
        }

    @classmethod
    def render_docs(cls):
        if hasattr(cls, 'doc_template') and cls.doc_template:
            template = loader.get_template(cls.doc_template, using='docs')
            context = cls._docs_context()
            for method in cls.allowed_methods:
                method_cls = getattr(cls, method, None)
                context[method] = method_cls._docs_context() if method_cls else None
