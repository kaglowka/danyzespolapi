...
@pytest.mark.django_db
def test_save_model_given_creator_user(admin_user, admin_user2):
    obj = {
...
@pytest.mark.django_db
def test_add_tags_to_article(admin_user, valid_tag, valid_article):
    obj = {
        'title': "Test with dataset title",
        'slug': 'test-name',
        'notes': 'tresc',
...
@pytest.mark.django_db
def test_removed_articles_are_not_in_applicaiton_list(admin_user, valid_article):
    client = Client()
    client.force_login(admin_user)
