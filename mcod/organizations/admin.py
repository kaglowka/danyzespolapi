...
class AddDatasetStacked(nested_admin.NestedStackedInline, ObjectPermissionsStackedInline):
    ...
    form = AddDatasetForm
    suit_classes = 'suit-tab suit-tab-datasets'
    fields = ('title', 'notes', 'url', 'customfields', 'update_frequency', 'category', 'status',
...
class OrganizationAdmin(HistoryMixin, nested_admin.NestedModelAdmin, ObjectPermissionsModelAdmin):
    actions_on_top = True
    prepopulated_fields = {"slug": ("title",)}
    list_display = ["slug", "title", "get_photo", "short_description", "status", 'obj_history']
    list_filter = ["status"]
